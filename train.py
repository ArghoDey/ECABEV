...
import utils.vox
import wandb
from nets.segnet_ecabev import SegnetECABEV
from nets.ecabev_modules import ScalableCrossEntropyBEVLoss
...
def main(
        ...
        # ECABEV: scalable cross-entropy BEV loss for the object head
        use_sce_loss=True,
        sce_num_buckets=10,
        sce_top_k=4,
        # wandb
    ...
    # set up model & losses
    # ECABEV: use the Scalable Cross-Entropy (SCE) BEV loss for the object head
    # to handle class imbalance efficiently; fall back to plain BCE if disabled.
    if use_sce_loss:
        seg_loss_fn = ScalableCrossEntropyBEVLoss(
            num_buckets=sce_num_buckets, top_k=sce_top_k, pos_weight=2.13).to(device)
        print(f"Using Scalable Cross-Entropy BEV loss "
              f"(num_buckets={sce_num_buckets}, top_k={sce_top_k})")
    else:
    ...
    map_seg_loss_fn = SigmoidFocalLoss(alpha=0.25, gamma=3, reduction="sum_of_class_means").to(
        device)  # for map segmentation head (multi-class alpha-balanced focal loss)

    # Transformer based lifting and fusion
    # ECABEV: transformer-based two-way (camera + radar) BEV fusion network
    # augmented with de-noise enhanced channel attention and bilinear
    # interpolation layer normalization.
    model = SegnetECABEV(Z_cam=200, Y_cam=8, X_cam=200, Z_rad=Z, Y_rad=Y, X_rad=X, vox_util=None,
