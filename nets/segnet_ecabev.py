...
Main network of **ECABEV** (Enhanced Channel Attention BEV).

The architecture keeps the proven two-way (camera + radar) BEV encoder,
DINOv2 image backbone, deformable-attention lifting, and transformer fusion
of the BEVCar baseline, and augments it with the three ECABEV contributions
(see :mod:`nets.ecabev_modules`):

* **De-Noise Enhanced Channel Attention** applied to the camera BEV features
  before they are fused with radar (refines noisy camera data).
* **Bilinear Interpolation Layer Normalization** used to fuse the camera and
  radar BEV feature maps before the decoder (spatial fidelity, lower cost).
* **Scalable Cross-Entropy (SCE) BEV loss** is defined in the training script
  and used for the object head (class imbalance handling).

Original baseline: J. Schramm et al., "BEVCar: Camera-Radar Fusion for BEV Map
and Object Segmentation", IROS 2024.
"""
...
class SegnetECABEV(nn.Module):
    def __init__(self, Z_cam: int, Y_cam: int, X_cam: int, Z_rad: int, Y_rad: int, X_rad: int,
        ...
        super(SegnetECABEV, self).__init__()
        ...
        if self.use_radar:
            ...
            # -------- ECABEV contribution 1: de-noise enhanced channel attention -------- #
            # One channel-attention block per fusion layer. It re-calibrates the
            # camera BEV tokens (keys/values of the fusion cross-attention) so that
            # noisy channels are suppressed before fusion with radar.
            self.denoise_ca_fuser = nn.ModuleList([
                DeNoiseEnhancedChannelAttention(channels=latent_dim, reduction=8)
                for _ in range(num_layers)
            ])

            # -------- ECABEV contribution 2: bilinear interpolation layer norm --------- #
            # Fuses the camera and radar BEV feature maps (each (B, C, Z, X)) into a
            # single scale-consistent map before the decoder.
            self.bilinear_interp_ln = BilinearInterpolationLayerNorm(
                image_channels=latent_dim,
                radar_channels=latent_dim,
                fused_channels=latent_dim,
                out_size=(Z_cam, X_cam),
            )
    ...
    def forward(self, rgb_camXs: torch.Tensor, pix_T_cams: torch.Tensor, cam0_T_camXs: torch.Tensor,
        ...
        if self.use_radar:
            ...
            for i in range(self.num_layers):
                ...
                # ECABEV: de-noise enhanced channel attention on the camera BEV
                # features (keys/values of the fusion cross-attention). This
                # suppresses noisy channels before they are fused with radar.
                cam_feats_denoised = self.denoise_ca_fuser[i](bev_queries_cam_out)

                # fusing layer
                if self.use_radar_as_k_v:
                    bev_queries = self.fusing_attn_layers_fuser[i](bev_queries, rad_bev_query)  # rad_bev_query
                else:
                    bev_queries = self.fusing_attn_layers_fuser[i](bev_queries, cam_feats_denoised)
        ...
        if self.use_radar:
            # camera BEV map from the encoder branch (B, C, Z, X)
            cam_bev_map = bev_queries_cam_out.permute(0, 2, 1).reshape(
                B, self.latent_dim, self.Z_cam, self.X_cam)
            # fused (camera+radar) BEV map from the fuser branch (B, C, Z, X)
            fused_bev_map = bev_queries_fuser_out.permute(0, 2, 1).reshape(
                B, self.latent_dim, self.Z_cam, self.X_cam)
            # ECABEV: bilinear interpolation layer normalization fuses the camera
            # and radar(-fused) BEV maps into a scale-consistent representation.
            feat_bev_ = self.bilinear_interp_ln(image_feat=cam_bev_map, radar_feat=fused_bev_map)
        else:
